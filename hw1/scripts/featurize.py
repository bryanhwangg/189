...
def freq_cap_feature(text, freq):
...
def generate_feature_vector(text, freq):
    ...
    feature.append(freq_cap_feature(text, freq))
...
test_filenames = [
    BASE_DIR + TEST_DIR + str(x) + ".txt" for x in range(NUM_TEST_EXAMPLES)
]
test_design_matrix = generate_design_matrix(test_filenames)

X = spam_design_matrix + ham_design_matrix
Y = (
    np.array([1] * len(spam_design_matrix) + [0] * len(ham_design_matrix))
    .reshape((-1, 1))
    .squeeze()
)

np.savez(
    BASE_DIR + "spam-data.npz",
    training_data=X,
    training_labels=Y,
